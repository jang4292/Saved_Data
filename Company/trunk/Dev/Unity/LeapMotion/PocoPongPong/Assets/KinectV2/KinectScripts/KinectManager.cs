...
public class KinectManager : MonoBehaviour 
{
	...
	public bool ignoreJointZCoords = false;
...
//    }
	...
	private void UpdateKinectStreams()
	{
		if(kinectInitialized)
		{
			...
			if(sensorData == null || !sensorData.isPlayModeEnabled)
			{
				sensorData.newBodyFrame = KinectInterop.PollBodyFrame(sensorData, ref bodyFrame, ref kinectToWorld, ignoreJointZCoords);
			}
			else
			{
            ...
            // 자체 프레임 조절을 안받고 바로 필터링 되게 메소드를 뺌
            if (smoothing != Smoothing.None)
            {
                jointPositionFilter.UpdateFilter(ref bodyFrame);
            }
