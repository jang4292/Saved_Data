...
//	 private final static String TAG = NavigationActivity.class.getSimpleName();
	private final static String TAG = "ble_terminal";
...
//	 public int title_heigh=0;
	...
	private void AlerEditor() {
		...
		LinearLayout linearLayoutView = new LinearLayout(MainActivity.Mcontext);
		LinearLayout linearLayoutView1 = new LinearLayout(MainActivity.Mcontext);
		LinearLayout linearLayoutView2 = new LinearLayout(MainActivity.Mcontext);
		...
		final Spinner font_list = new Spinner(MainActivity.Mcontext);
		final Spinner echo_list = new Spinner(MainActivity.Mcontext);
		final TextView tv1 = new TextView(MainActivity.Mcontext);//
		final TextView tv2 = new TextView(MainActivity.Mcontext);//
		...
		ArrayAdapter<String> spinnerArrayAdapter = new ArrayAdapter<String>(MainActivity.Mcontext,   android.R.layout.simple_spinner_item, fontSizeItems);
		...
		ArrayAdapter<String> echo_listArrayAdapter = new ArrayAdapter<String>(MainActivity.Mcontext,   android.R.layout.simple_spinner_item, echoItems);
	...
	private ArrayList<String> alByteToString = new ArrayList<>();
	...
	public void dataReceived(String  uudi_data, String data_string,byte[] row_data){
		// TODO Auto-generated method stub
		//Log.d("Test", "dataReceived : uudi_data : "+ uudi_data);
		//Log.d("Test", "dataReceived : data_string : "+ data_string);
		//Log.d("Test", "dataReceived : row_data : "+ row_data.toString());
		//Log.d("Test", "dataReceived : row_data String : "+ new String(row_data));

		alByteToString.add(new String(row_data));

		Log.d("Test", "[uudi_data] : " + uudi_data);
		//Log.d("Test", "[Data_String] : " + data_string);

		Log.d("Test", "[Data_String] : " + new String(data_string));
		if(alByteToString.size() > 2) {
			if(alByteToString.get(0).contains("<") && alByteToString.get(alByteToString.size() - 1).contains(">")) {
				StringBuffer stringBuffer = new StringBuffer();
				for (String str : alByteToString) {
					stringBuffer.append(str);
				}
				Log.d("Test", "[byteString] : " + stringBuffer.toString());
				check(stringBuffer.toString());
				alByteToString.clear();
			}
		}

		/*
		String str = new String(row_data);
		if(str.contains("<")) {
			...
			if(!byteString.isEmpty()) {
				byteString += str;
				Log.d("Test", "[byteString] : "+ byteString);
				byteString = "";
			}
		} else {
			if(!byteString.isEmpty()) {
		...
		*/
		...
		//String byteToString = new String(row_data,0,row_data.length);

		//Log.d("Test", "byteToString : " + byteToString);
		/*
	...
	private String removeString(String str, String[] remover) {
...
/*	private void check(String str) {
		...
		Log.d("Test", "[ByteStringEach] Start");
		for (String s : strl) {
			Log.d("Test", "[ByteStringEach] : " + s);
		}
		Log.d("Test", "[ByteStringEach] End");
	}*/
...
class TranscriptScreen implements Screen {
	...
	private void init(int columns, int totalRows, int screenRows,
		...
		//Log.d("Test", "init mData1 : " + mData);
		mData = new char[totalSize];
		//Log.d("Test", "init mData2 : " + mData);
	...
	public void blockCopy(int sx, int sy, int w, int h, int dx, int dy) {
		...
		if (sy <= dy) {
			...
			for (int y = 0; y < h; y++) {
				...
				Log.d("Test", "System.arraycopy1 mData : " + mData);
			...
			for (int y = 0; y < h; y++) {
				...
				Log.d("Test", "System.arraycopy2 mData : " + mData);
				System.arraycopy(mData, srcOffset, mData, dstOffset, w);
			}
		}
	}
	...
	public void blockSet(int sx, int sy, int w, int h, int val, int foreColor,
		...
		//Log.d("Test", "blockSet mData : " + mData);
		char[] data = mData;
		char encodedVal = encode(val, foreColor, backColor);
		//Log.d("Test", "encodedVal : " + encodedVal);
		for (int y = 0; y < h; y++) {
	...
	public final void drawText(int row, Canvas canvas, float x, float y,
		...
		for (int i = 0; i < columns; i++) {
			...
			if (colors != lastColors) {
				if (lastRunStart >= 0) {
					//Log.d("Test", "846 rowBuffer : " + rowBuffer);
		...
		if (lastRunStart >= 0) {
			//Log.d("Test", "856 rowBuffer : " + rowBuffer);
			renderer.drawTextRun(canvas, x, y, lastRunStart, rowBuffer,
	...
	private String internalGetTranscriptText(boolean stripColors) {
		...
		Log.d("Test", "builder.toString() : " + builder.toString());
...
class PaintRenderer extends BaseTextRenderer {
	...
	public void drawTextRun(Canvas canvas, float x, float y, int lineOffset,
		...
		//Log.d("Test", "text.length : " + text.length);
		//Log.d("Test", "text.toString() : " + text.toString());


		//Log.d("Test", "Arrays.toString(text) : " + Arrays.toString(text));
...
class EmulatorView extends View implements GestureDetector.OnGestureListener {
	...
	public String getTranscriptText() {
		...
		Log.d("Test", "mEmulator.getTranscriptText() : " + mEmulator.getTranscriptText());
	...
	public void write(byte[] buffer, int length) {
		//length
		//Log.d("Test", "write buffer : " + buffer);
		//Log.d("Test", "write length : " + length);
		try {
	...
	public void commonConstructor(Context context) {
		//Log.d("Test", "2760 mTextRenderer : " + mTextRenderer);
		mTextRenderer = null;
	 ...
	 */
	public void append(byte[] buffer, int base, int length) {
		Log.d("Test", "ble_termianl_append 1");
	...
	private void updateText() {
		...
		if (mTextSize > 0) {
			...
			mTextRenderer = new PaintRenderer(mTextSize, mForeground,
					mBackground);
			//Log.d("Test", "2989 mTextRenderer : " + mTextRenderer);
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = mTopRow; i < endLine; i++) {
			...
			//Log.d("Test", "3084 mTextRenderer : " + mTextRenderer);
